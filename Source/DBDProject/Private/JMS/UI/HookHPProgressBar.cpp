...
#include "JMS/UI/HookHPProgressBar.h"
...
void UHookHPProgressBar::TagChanged(const FGameplayTag Tag, int32 NewCount)
{
	if (NewCount>0)
	{
...
void UHookHPProgressBar::SetPercent(float Percent)
{
	ProgressBar->SetPercent(Percent);
}

void UHookHPProgressBar::AttributeChanged(const FOnAttributeChangeData& Data)
{
	SetPercent(Data.NewValue / CachedMaxValue);
}

void UHookHPProgressBar::NativePreConstruct()
{
	Super::NativePreConstruct();
}

void UHookHPProgressBar::NativeConstruct()
{
	...
	if (OwnerASC)
	{
		...
		OwnerASC->GetGameplayAttributeValueChangeDelegate(CurrentValueAttribute).AddUObject(
			this, &UHookHPProgressBar::AttributeChanged);
		OwnerASC->RegisterGameplayTagEvent(TargetTag).AddUObject(this, &UHookHPProgressBar::TagChanged);
	}
}
