...
public:
	...
	UFUNCTION()
	void OnRep_WalkingSpeed(const FGameplayAttributeData& OldValue) const;
	
	//살인마 기절 시간(애니메이션 재생 Rate)
	UPROPERTY(ReplicatedUsing=OnRep_StunRate)
	FGameplayAttributeData StunRate;
	ATTRIBUTE_ACCESSORS(UKillerAttributeSet, StunRate);
	UFUNCTION()
	void OnRep_StunRate(const FGameplayAttributeData& OldValue) const;

	//생존자 드는 시간(애니메이션 재생 Rate)
	UPROPERTY(ReplicatedUsing=OnRep_PickUpSpeedRate)
	FGameplayAttributeData PickUpSpeedRate;
	ATTRIBUTE_ACCESSORS(UKillerAttributeSet, PickUpSpeedRate);
	UFUNCTION()
	void OnRep_PickUpSpeedRate(const FGameplayAttributeData& OldValue) const;

	//살인마 공포 범위
	//4.6m/s -> 32m | 4.4m/s -> 24m
	UPROPERTY(ReplicatedUsing=OnRep_TerrorRadiusRange)
	FGameplayAttributeData TerrorRadiusRange;
	ATTRIBUTE_ACCESSORS(UKillerAttributeSet, TerrorRadiusRange);
	UFUNCTION()
	void OnRep_TerrorRadiusRange(const FGameplayAttributeData& OldValue) const;

	//판자 부수기 속도(애니메이션 재생 Rate)
	UPROPERTY(ReplicatedUsing=OnRep_DestroyPalletRate)
	FGameplayAttributeData DestroyPalletRate;
	ATTRIBUTE_ACCESSORS(UKillerAttributeSet, DestroyPalletRate);
	UFUNCTION()
	void OnRep_DestroyPalletRate(const FGameplayAttributeData& OldValue) const;

	//발전기 부수기 속도
	UPROPERTY(ReplicatedUsing=OnRep_DamageGeneratorRate)
	FGameplayAttributeData DamageGeneratorRate;
	ATTRIBUTE_ACCESSORS(UKillerAttributeSet, DamageGeneratorRate);
	UFUNCTION()
	void OnRep_DamageGeneratorRate(const FGameplayAttributeData& OldValue) const;
